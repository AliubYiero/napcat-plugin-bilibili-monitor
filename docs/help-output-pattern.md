...
- **按角色与会话类型输出对应版本**: 帮助内容分 User / Admin / SuperAdmin 三个权限组, 由消息来源的角色与会话类型共同决定输出版本 (见第 5 节)。
- **图片优先 / 文本回退**: 优先发送帮助图片, 图片文件缺失或发送失败时回退发送纯文本帮助。
...
上游工具项目为 `napcat-help-generate` (纯前端页面, 编辑 `cmd.json` 后导出产物), 其配置类型详见该项目 `type.ts` 与 README。
...
```typescript
interface Cmd {
    id: string;     // 脚注, 同时用作导出文件名 (如 bilibili-live-monitor)
    title: string;  // 面板标题 (如 "Bilibili Live Monitor 插件帮助")
...
### 2.3 SuperAdmin 版的内容取舍

SuperAdmin 版可以包含**仅私聊可用**的指令 (`onlyPrivate`)。这直接影响下游变体映射的合理性: 群聊中不输出 SuperAdmin 版, 正是因为其含有的仅私聊指令在群聊中不可用 (见第 5 节)。编写上游配置时保持这一内容构成约定。
...
1. 在上游工具项目 `cmd.json` 中编写该帮助配置 (遵循第 2 节规范)。
2. 通过 help-generate 页面导出 PNG 与 texts.json (第 3 节)。
