...
本文是适用于任意 NapCat 插件项目的消息发送通用范式，覆盖发送 API 的选型规则、消息段的构造方式，以及对应的反例。各项目遵循此范式时，应将所有发送工具集中在一个工具模块中（本文以 `src/handlers/utils.ts` 为例，下称"发送工具模块"）。
...
- 四个函数统一返回 `Promise<boolean>`：内部 try/catch，失败时记录日志（如 `pluginState.logger.error`）并返回 `false`，**不向调用方抛出**。
...
发送工具函数内部依赖 `pluginState` 的 logger 等全局状态（来自 `src/core/state.ts` 的单例）。其安全性前提与数据存储范式的约束一致（参见 `store-pattern.md`）：所有工具函数只在 `plugin_init` 之后的运行期被调用，因此可以安全访问 `pluginState`；**不要在模块加载期调用任何发送函数**。
