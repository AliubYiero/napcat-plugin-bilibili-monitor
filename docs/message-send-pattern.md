...
本文是适用于任意 NapCat 插件项目的消息发送通用范式, 覆盖发送 API 的选型规则、消息段的构造方式, 以及对应的反例。各项目遵循此范式时, 应将所有发送工具集中在一个工具模块中 (本项目为 `src/handlers/utils.ts`, 下称"发送工具模块")。
...
1. **发送必须走发送工具模块提供的四个工具函数**, 禁止在业务代码中直接调用 `ctx.actions.call('send_msg' | 'send_group_msg' | 'send_private_msg' | ...)`。
2. **复杂消息 (多段消息、含图片/表情等消息段的消息) 必须用 `createXxxMessage` 工厂函数构造**, 禁止在业务代码中手写 `{ type: 'text', data: { ... } }` 字面量 (工厂未覆盖的消息段类型除外, 见下文)。

## 范式正文

### 发送工具模块应提供的 API: 按场景三分法

发送工具模块应提供以下四个函数, 各有明确的适用场景, 按"是否持有 event"和"目标类型"选择:
...
- 四个函数统一返回 `Promise<boolean>`: 内部 try/catch, 失败时记录日志并返回 `false`, **不向调用方抛出**。
...
需要消息段 (@、回复、图片、表情等) 或多段组合时, 用 `createXxxMessage` 工厂构造后再发送:
...
**范式覆盖的消息段范围即上表所列 8 种, 全部提供工厂函数**。范围外的消息段类型 (markdown、record、video、location、json、music 等) 不属于本范式覆盖范围, 各项目如需支持应先扩展工厂函数再使用。
...
- 混合字符串与消息段时, 统一升级为消息段数组 (字符串那一部分用 `createTextMessage`), 不要在一个数组里混放字符串与消息段对象。
- 范围内的 8 种消息段类型一律通过工厂函数构造; 确需使用范围外类型时才允许手写字面量, 且必须能通过 `OB11MessageData` 联合类型的类型检查。
...
发送函数失败时**不会抛出** (内部已 try/catch、记日志、返回 `false`), 因此调用方是否检查返回值是弹性的:

- **推荐检查**的场景: 指令响应、用户可见的失败提示等关键路径——发送失败后往往需要后续逻辑 (如提示用户重试)。
- **可忽略返回值**的场景: 日志性质的通知、状态推送——失败已被工具函数记录日志, 静默即可。

无需在调用点重复 try/catch 或重复记日志; 工具函数已统一处理。

## 标准范式清单

发送一条消息时按以下步骤:

1. 有 `event` → `sendReply(ctx, event, message)`; 无 `event` 按目标选 `sendGroupMessage` / `sendPrivateMessage`; 合并转发用 `sendForwardMsg`。
2. 纯文本消息直接传字符串, 不构造消息段。
3. 需要 @ / 回复 / 图片 / 表情 / 文件或多段组合 → 用 `createXxxMessage` 工厂构造成消息段数组再发送。
4. 禁止在业务代码中直接调用 `ctx.actions.call('send_msg' | ...)`。
5. 范围内消息段一律用工厂构造; 范围外类型允许手写字面量, 且必须通过 `OB11MessageData` 类型检查。
6. 不重复 try/catch 发送逻辑; 关键路径按需检查返回的 `boolean`, 通知类可忽略。

## 与其他范式的关系

- 与**生命周期铁律**的关系: 发送工具函数内部依赖全局状态的 logger 等, 只在 `plugin_init` 之后的运行期被调用; **不要在模块加载期调用任何发送函数**。前提约束见 `store-pattern.md` 与 `development-pattern.md`。
- 与**指令分发范式**的关系: handler 回复用户用 `sendReply`; 主动推送 (无 event) 按目标类型选 `sendGroupMessage` / `sendPrivateMessage`。见 `instruction-pattern.md`。
